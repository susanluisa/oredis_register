...
import { createSocioeconomicStatus, deleteSocioeconomicStatus, updateSocioeconomicStatus } from "../catalogos/services/socioeconomic_status_services";
import { useSocioeconomicStatuses } from "@/hooks/use-socioeconomic-status";

const columns = (
  onEdit: (item: SocioeconomicStatusType) => void,
  onDelete: (id: number) => Promise<void>
): ColumnDef<SocioeconomicStatusType>[] => [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex gap-2 justify-center">
          ...
          <DeleteModal title="¿Eliminar situación socioeconómica?" entityLabel={`${row.original.name}`} onConfirm={handleDelete} />
        </div>
      );
    },
  },
...
export default function SocioeconomicStatusesTable() {
  const queryClient = useQueryClient();
  const { data: items = [], isLoading } = useSocioeconomicStatuses();
  ...
  const handleDelete = async (id: number) => {
    try {
      await deleteSocioeconomicStatus(id);
      queryClient.invalidateQueries({ queryKey: ["socioeconomic_statuses"] });
    } catch (error) {
      console.error("Error eliminando situación socioeconómica:", error);
    }
  };
      ...
      <div className="flex items-start p-4">
        <form
          ...
          onSubmit={async (e) => {
            ...
            try {
              setSubmitting(true);
              const payload = { code: c, name: n, order: o, is_active: isActive } as Partial<SocioeconomicStatusType>;
              if (editingId) {
                await updateSocioeconomicStatus(editingId, payload);
              } else {
                await createSocioeconomicStatus(payload);
              }
              setCode("");
              setName("");
              setOrder("");
        ...
        >
          <h3 className="font-semibold">{editingId ? "Editar situación socioeconómica" : "Crear situación socioeconómica"}</h3>
          <div className="flex flex-col gap-2">
            <label className="text-sm text-muted-foreground">Código</label>
            <Input value={code} onChange={(e) => setCode(e.target.value)} placeholder="Ej. NSE2" disabled={submitting} />
          </div>
          <div className="flex flex-col gap-2">
            <label className="text-sm text-muted-foreground">Nombre</label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ej. Medio" disabled={submitting} />
          </div>
