...
export const getApiBaseUrl = (): string => {
  ...
  return "https://custom_domain.com";
};
...
// Si hay token en localStorage, configúralo como Authorization por defecto
...
/* Interceptor para refrescar automáticamente el token */
apiPrivateClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if ([401, 403].includes(error.response.status)) {
      console.log("****-*-*-*-*--*-*-*", apiPrivateClient.defaults.headers.common.Authorization)
